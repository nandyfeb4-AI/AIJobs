...
type RegistryStats = {
  ...
  jobs: {
    ...
    byWorkMode: CountRow[];
    byLocation: CountRow[];
    byCountryScope: CountRow[];
    qualityFlags: CountRow[];
...
function countByLabel(rows: CountRow[] | undefined, labels: string[]) {
  if (!rows) return 0;
  const wanted = new Set(labels);
  return rows.reduce(
    (sum, row) => (wanted.has(row.label) ? sum + row.count : sum),
    0,
  );
}
...
function StatTile({
  ...
  return (
    <div
      className={`${toneClass} rounded-xl px-5 py-4 shadow-[0_1px_6px_rgba(26,32,24,0.06)]`}
    >
      <p className="text-sage text-[10px] uppercase tracking-[0.14em] mb-2">
        {label}
      </p>
...
function Breakdown({
  ...
  return (
    <section className="bg-card rounded-2xl p-5 shadow-[0_2px_12px_rgba(26,32,24,0.06)]">
      <h2 className="text-ink text-sm font-semibold tracking-tight mb-4">
        {title}
      </h2>
      <div className="space-y-3">
        {visibleRows.length ? (
          visibleRows.map((row) => {
            const width = total
              ? Math.max(4, Math.round((row.count / total) * 100))
              : 0;
            return (
              <div key={row.label}>
...
export function JobRegistryShell() {
  ...
  const [feedRunning, setFeedRunning] = useState<"dry-run" | "ingest" | null>(
    null,
  );
  const [feedResult, setFeedResult] = useState<WorkableXmlFeedResult | null>(
    null,
  );
  ...
  async function runWorkableXmlFeed(dryRun: boolean) {
    try {
      ...
      const response = await fetch(
        `${apiBase()}/jobs/feeds/workable-xml/ingest`,
        {
          method: "POST",
          headers: {
  ...
  const postedOlderThan60 =
    stats?.jobs.postedAgeBuckets.find((row) => row.label === "60+ days")
      ?.count ?? 0;
  ...
  const usScopedJobs = countByLabel(stats?.jobs.byCountryScope, [
    "US",
    "Remote US",
    "North America",
  ]);
  const nonUsVisibleJobs = countByLabel(stats?.jobs.qualityFlags, [
    "Non-US visible",
  ]);
  const uncategorizedJobs = countByLabel(stats?.jobs.qualityFlags, [
    "Uncategorized role",
  ]);
  ...
  return (
    <div className="px-8 py-8 max-w-7xl mx-auto space-y-6">
      <header className="flex flex-col gap-3 md:flex-row md:items-end md:justify-between">
        <div>
          ...
          <p className="text-sage text-sm">
            Inventory health for active jobs, freshness, categories, and board
            sync coverage.
          </p>
        </div>
      ...
      {error ? (
        <section className="bg-card rounded-2xl p-5 shadow-[0_2px_12px_rgba(26,32,24,0.06)]">
          <p className="text-sm text-red-700">
            Could not load job registry: {error}
          </p>
        </section>
      ) : null}
      ...
      <section className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-6 gap-3">
        ...
        <StatTile
          label="US Scope"
          value={loading && !stats ? "..." : formatNumber(usScopedJobs)}
          caption={`${formatPercent(usScopedJobs, activeJobs)} US / Remote-US / NA`}
          tone={nonUsVisibleJobs ? "warning" : "positive"}
        />
        <StatTile
          label="Quality Flags"
          value={
            loading && !stats
              ? "..."
              : formatNumber(nonUsVisibleJobs + uncategorizedJobs)
          }
          caption={`${formatNumber(nonUsVisibleJobs)} non-US · ${formatNumber(uncategorizedJobs)} other roles`}
          tone={nonUsVisibleJobs ? "warning" : "neutral"}
        />
        <StatTile
      ...
      <section className="grid grid-cols-1 lg:grid-cols-2 xl:grid-cols-4 gap-4">
        <Breakdown
          title="Jobs by ATS"
          rows={stats?.jobs.bySource ?? []}
          total={activeJobs}
        />
        <Breakdown
          title="Jobs by Category"
          rows={stats?.jobs.byCategory ?? []}
          total={activeJobs}
        />
        <Breakdown
          title="Country Scope"
          rows={stats?.jobs.byCountryScope ?? []}
          total={activeJobs}
        />
        <Breakdown
          title="Work Mode"
          rows={stats?.jobs.byWorkMode ?? []}
          total={activeJobs}
        />
      </section>
      ...
      <section className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        <Breakdown
          ...
          rows={stats?.jobs.byLocation ?? []}
          total={activeJobs}
        />
        <Breakdown
          title="Data Quality Flags"
          rows={stats?.jobs.qualityFlags ?? []}
          total={activeJobs}
        />
      </section>
      ...
      <section className="grid grid-cols-1 lg:grid-cols-2 xl:grid-cols-[minmax(0,1fr)_minmax(0,1fr)_260px] gap-4">
        <Breakdown
          title="Posted Age Buckets"
          rows={stats?.jobs.postedAgeBuckets ?? []}
          total={activeJobs}
          limit={6}
        />
        <Breakdown
          title="Last Sync Buckets"
          rows={stats?.jobs.syncAgeBuckets ?? []}
          total={activeJobs}
          limit={6}
        />
        <section className="bg-card rounded-2xl p-5 shadow-[0_2px_12px_rgba(26,32,24,0.06)]">
          ...
          <div className="space-y-4">
            <div>
              <p className="text-sage text-[10px] uppercase tracking-[0.14em]">
                Core 30D
              </p>
              ...
              <p className="text-sage text-xs">
                {formatPercent(
                  stats?.jobs.freshness.posted30d ?? 0,
                  activeJobs,
                )}{" "}
                active jobs
              </p>
            ...
            <div className="border-t border-line pt-4">
              ...
              <p className="text-ink text-2xl font-semibold tabular-nums">
                {formatNumber(posted60d)}
              </p>
              <p className="text-sage text-xs">
                {formatNumber(postedOlderThan60)} older than 60D
              </p>
            </div>
          </div>
        </section>
        ...
        <section className="bg-card rounded-2xl shadow-[0_2px_12px_rgba(26,32,24,0.06)] overflow-hidden">
          <div className="px-6 py-5 border-b border-line">
            ...
            <p className="text-sage text-xs mt-1">
              Latest active records refreshed by board ingestion.
            </p>
          </div>
            ...
            <table className="w-full min-w-[760px]">
              ...
              <tbody>
                {(stats?.jobs.recent ?? []).map((job, index) => (
                  <tr
                    key={`${job.sourceKey}:${job.lastSyncedAt}:${index}`}
                  ...
                  >
                    <td className="px-6 py-4">
                      ...
                      <p className="text-xs text-sage mt-1">
                        {job.roleCategory} · {job.boardToken ?? "unknown board"}
                      </p>
                    </td>
                    <td className="px-4 py-4 text-sm text-ink">
                      {job.company}
                    </td>
                    <td className="px-4 py-4 text-sm text-sage capitalize">
                      {job.sourceName}
                    </td>
                    <td className="px-4 py-4 text-sm text-sage">
                      <p>{job.location ?? job.remoteType ?? "Unknown"}</p>
                      <p className="text-xs mt-1">
                        {job.countryScope} · {job.workMode}
                      </p>
                    </td>
                    <td className="px-4 py-4 text-sm text-sage">
                      {formatDate(job.lastSyncedAt)}
                    </td>
                  </tr>
                ))}
                {!loading && stats?.jobs.recent.length === 0 ? (
                  <tr>
                    <td
                      colSpan={5}
                      className="px-6 py-8 text-center text-sm text-sage"
                    >
                      No persisted jobs yet. Run board verification/ingest to
        ...
        <section className="space-y-4">
          <section className="bg-card rounded-2xl p-5 shadow-[0_2px_12px_rgba(26,32,24,0.06)]">
            <div className="mb-4">
              ...
              <p className="text-sage text-xs mt-1">
                Separate from board tracking. Streams the public Workable feed,
                then keeps only fresh US target-role jobs.
              </p>
            </div>

            <div className="grid grid-cols-3 gap-3">
              <label className="block">
                <span className="text-[10px] uppercase tracking-[0.12em] text-sage">
                  Fresh Days
                </span>
                <input
                  type="number"
                  ...
                  value={feedFreshDays}
                  onChange={(event) =>
                    setFeedFreshDays(Number(event.target.value))
                  }
              ...
              <label className="block">
                ...
                <input
                  ...
                  onChange={(event) =>
                    setFeedMaxRecords(Number(event.target.value))
                  }
                  className="mt-1 w-full rounded-lg border border-line bg-parchment px-3 py-2 text-sm text-ink outline-none"
                />
              </label>
            ...
            {feedResult ? (
              <div className="mt-4 rounded-xl border border-line bg-parchment p-4">
                <div className="grid grid-cols-2 gap-3 text-xs">
                  <div>
                    <p className="text-sage uppercase tracking-[0.12em]">
                      Matched
                    </p>
                  ...
                  <div>
                    ...
                    <p className="text-ink text-lg font-semibold">
                      {feedResult.dryRun ? "Dry run" : "Saved"}
                    </p>
                  </div>
                  <div>
                    <p className="text-sage uppercase tracking-[0.12em]">
                    ...
                    <p className="text-ink font-medium tabular-nums">
                      {formatNumber(feedResult.inserted)}
                    </p>
                  </div>
                  <div>
                    <p className="text-sage uppercase tracking-[0.12em]">
                    ...
                    <p className="text-ink font-medium tabular-nums">
                      {formatNumber(feedResult.updated)}
                    </p>
                  </div>
                  <div>
                    <p className="text-sage uppercase tracking-[0.12em]">
                    ...
                    <p className="text-ink font-medium tabular-nums">
                      {formatNumber(feedResult.skippedDuplicate)}
                    </p>
                  </div>
                  <div>
                    <p className="text-sage uppercase tracking-[0.12em]">
                    ...
                    <p className="text-ink font-medium tabular-nums">
                      {formatNumber(feedResult.skippedPersistError)}
                    </p>
                  </div>
                </div>
                <p className="text-sage text-xs mt-3">
                  Scanned {formatNumber(feedResult.seen)} records. Skipped{" "}
                  {formatNumber(feedResult.skippedOld)} old,{" "}
                  {formatNumber(feedResult.skippedNonUs)} non-US, and{" "}
                  {formatNumber(feedResult.skippedNonTarget)} non-target.
                </p>
                {feedResult.persistErrors.length ? (
                  <p className="text-red-700 text-xs mt-3">
                    First write issue: {feedResult.persistErrors[0]?.company} /{" "}
                    {feedResult.persistErrors[0]?.title} -{" "}
          ...
          <section className="bg-card rounded-2xl p-5 shadow-[0_2px_12px_rgba(26,32,24,0.06)]">
            <h2 className="text-ink text-sm font-semibold tracking-tight mb-4">
              Board Sync Freshness
            </h2>
            <div className="grid grid-cols-2 gap-3">
              <StatTile
                label="Checked 24H"
                value={formatNumber(stats?.boards.freshness.checked24h ?? 0)}
                caption="boards"
              />
              <StatTile
                label="Checked 7D"
                value={formatNumber(stats?.boards.freshness.checked7d ?? 0)}
                caption="boards"
              />
              <StatTile
                label="Older 14D"
                value={formatNumber(stats?.boards.freshness.olderThan14d ?? 0)}
                caption="boards"
                tone="warning"
              />
              <StatTile
                label="Never"
                value={formatNumber(stats?.boards.freshness.neverChecked ?? 0)}
                caption="boards"
                tone="warning"
              />
            </div>
          ...
          <Breakdown
            title="Top Companies by Active Jobs"
            rows={stats?.jobs.topCompanies ?? []}
            total={activeJobs}
            limit={10}
          />
        </section>
